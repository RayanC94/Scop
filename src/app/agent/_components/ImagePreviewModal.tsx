  ...
  const handleCopyImage = async () => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        } else { throw new Error('Could not get canvas context'); }
      };
      img.onerror = () => { throw new Error('Image could not be loaded for copying'); }
    } catch (error) {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="data-[state=open]:animate-contentShow fixed top-1/2 left-1/2 w-[90vw] max-w-4xl max-h-[90vh] flex flex-col -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white shadow-lg focus:outline-none z-50 p-4">
          ...
          <div className="flex-shrink-0 mt-4 flex items-center justify-center w-full space-x-4">
            <button onClick={handleCopyImage} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">
              <Clipboard className="w-4 h-4" />
              {copyStatus === 'success' ? 'Copié !' : copyStatus === 'error' ? 'Erreur' : 'Copier l\'image'}
            </button>
            <button onClick={handleDownload} disabled={downloadStatus === 'loading'} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors disabled:opacity-50">
